  ...
  const BookingTable = () => {
    ...
    const handleSearch = async () => {
      try {
        const results = await getAvailableVehicles(searchForm);
          ...
          <tbody>
            {bookings.map((booking) => (
              <tr key={booking._id}>
                <td>
                  {
                    vehicles.find((v) => v._id === booking.vehicleId)
                      ?.vehicleName || "Unknown"
                  }
                </td>
          ...
          <form onSubmit={handleFormSubmit}>
            {fields.map((f) => (
              <div key={f.name} className="form-group">
                <label>{f.label}</label>
                {f.name === "vehicleId" && f.type === "select" ? (
                  <select
                    name="vehicleId"
          ...
          <div className="view-details">
            {fields.map((f) => (
              <p key={f.name}>
                <b>{f.label}:</b>{" "}
                {f.name === "vehicleId"
                  ? vehicles.find((v) => v._id === selectedBooking?.vehicleId)
                      ?.vehicleName || "Unknown"
                  : f.name === "startTime"
